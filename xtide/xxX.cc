// $Id: xxX.cc 8650 2025-06-16 14:27:16Z flater $
...
static const Pixmap makePixmap (char **data,
  ...
  int retval = XCreatePixmapFromData (xxX::display, xxX::rootWindow,
    data, &temp, NULL, &xpma);
  quashWarning(-Wimplicit-fallthrough)
  switch (retval) {
  case XpmColorError:
...
const Widget xxX::connect (int &argc, char **argv) {
  ...
  require (putenv (env_string) == 0);
    ...
    // Encoding is Latin-1.
    xaw3dxft_data->encoding = 8;

    // Get rid of extra vertical spacing in menus.
    xaw3dxft_data->menu_spacing = 0;

    // Provide a multicolumn menu for the Set Time dialog when possible.
    xaw3dxft_data->multi_column_menu = 1;

    // Multicolum menus are supported only when no_hilit_reverse == 1.  This
    // limitation might be removed in a future version of libXaw3dXft.
    // no_hilit_reverse affects highlighting of clicked list items and
    // moused-over menu items.
    xaw3dxft_data->no_hilit_reverse = 1;

    // Bitwise XOR value used to highlight list or menu items when
    // no_hilit_reverse == 1.
    xaw3dxft_data->proc->set_hilit_color((char*)"#332211");

    // Use more modern convention when Delete or another key is pressed while
    // editable text is selected.  Delete action = delete selected text;
    // other key action = replace selected text with char.
    xaw3dxft_data->edit_delete_alternative = 2;

    // Highlight selected text.
    xaw3dxft_data->text_bg_hilight = 1;

    // Bitwise XOR value applied to background colors to highlight selected
    // text when text_bg_hilight == 1.  This value causes a white background
    // to turn yellow.
    xaw3dxft_data->text_bg_hilight_color = 0x0000FF;
  #endif
...
void xxX::connectPostSettingsHook (Widget rootWidget) {
  ...
  // Schumacher is now a soft dependency; this can be null.  The package
  // containing Schumacher fonts is marked optional in Gentoo and Arch.
  iconFontStruct = XLoadQueryFont (display, iconFontName);
...
// Create a ClientSide::Font by scraping pixmaps (eww).  Based on a function
// by Jean-Pierre Demailly, 2012.  This is redundant with and inferior to
// what FreeType can do directly, but FreeType is an optional dependency that
// is linked only when HAVE_XAW3DXFT.
//
...
void xxX::getClientSideFont (FontStruct *fs, ClientSide::Font &font_out) {
  ...
  #else
    // We get XCharStruct *fs->per_char for free, but its first element is
    // not necessarily NUL (0x00).  Don't be clever; be consistent.
    XCharStruct extents[256];
    for (unsigned c=32; c<256; (c == 127 ? c=160 : ++c)) {
      if (c >= fs->min_char_or_byte2)
	extents[c] = fs->per_char[c - fs->min_char_or_byte2];
      else
	extents[c] = {0, 0, 0, 0, 0, 0};
    }
  #endif
